...
wav_file_path='/home/holla/Downloads/81.wav' # path to single wav file
...
then
	...
	python ../../utils/inference/single_file_inference.py -m ${custom_model_path} -d ${dictionary} -w ${wav_file_path} -c ${cuda} -D ${decoder} -l ${lexicon_lst_path} -L ${lm_model_path} -H ${half} 
fi
