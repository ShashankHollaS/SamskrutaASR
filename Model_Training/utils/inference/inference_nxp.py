...
class W2lDecoder(object):
    ...
    def generate(self, models, sample, **unused):
        ...
        # torch.onnx.export(models, encoder_input, 'onnx_model.onnx', export_params=True, opset_version=11, do_constant_folding=True, input_names = ['input'], output_names = ['output'])
...
def get_results(wav_path,dict_path,generator,use_cuda=False,w2v_path=None,model=None, half=None, **kwargs):
    sample = dict()
    net_input = dict()
    ...
    #print(type(waveform))
    #wav_in = [waveform]
    wav_in = tuple(waveform)
    # print("Model's state_dict:")
    # for param_tensor in model.state_dict():
    # 	print(param_tensor, "\t", model.state_dict()[param_tensor].size())
    #dict_val = dict(model.state_dict())
    # print(dict_val)
    # dict_val['mask_emb'] = dict_val.pop('w2v_encoder.w2v_model.mask_emb')
    # # first_pair = next(iter((dict_val.items())) )
    # model.load_state_dict(dict_val)
    #model.save()
    ...
    # print(type(dict_val))
    # print(first_pair)
    # key, val = dict_val.items()
    # df = pd.DataFrame(val.cpu().numpy())
    # df = df.to_json()
    # dict_val_new = dict(key, df)
    # # dict_val = dict_val.numpy()
    # with open('convert.txt', 'w') as convert_file:
    #     convert_file.write(json.dumps(dict_val_new))
        # for K, V in dict_val.items():
        #     convert_file.write(K + "\t" + V.cpu().numpy() + "\n")
    #traced_script_module = torch.jit.trace(model, wave_in, strict=False, check_trace=True)
    #traced_script_module = torch.jit.script(model, waveform)
    #print(traced_script_module)
    #model_out = model(feature)
    #torch.onnx.export(traced_script_module, wav_in, 'onnx_model.onnx', export_params=True, opset_version=11, do_constant_folding=True, input_names = ['input'], output_names = ['output'])
    ...
    wav_in = torch.randn(100).to(device)
    model_new = model.to(device)
    torch.onnx.export(model, wav_in, 'onnx_model.onnx', verbose=True, input_names = ['input'], output_names = ['output'])
